...
export default function GuruBkSiswaPage() {
    const { token } = useAuth();
    ...
    const fetchSiswa = async () => {
        try {
            const data = await apiFetch(`${API_URL}/siswa`, { headers: { 'Authorization': `Bearer ${token}` } });
            setSiswaList(data);
        } catch (err) {
        ...
        <div className="space-y-6 relative">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400 text-sm">Lihat data siswa terdaftar.</p>
                </div>
            </div>
