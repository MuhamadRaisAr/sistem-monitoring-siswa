...
import { 
    LayoutDashboard, BookOpen, Shield, HeartPulse, 
    CircleDollarSign, MessageSquare, LogOut, Menu, X, UserCheck,
    Sun, Moon, Key, UserCog, Eye, EyeOff, Camera, ChevronsLeft, ChevronsRight, GraduationCap, CalendarDays, Bell, BellOff
} from 'lucide-react';

function WaliLayoutContent({ children }) {
    const { user, logout, loading, token } = useAuth();
    const { theme, toggleTheme } = useTheme();
    const { childrenList, selectedChild, changeSelectedChild, loadingChildren } = useChild();
    ...
    useEffect(() => {
        ...
        const handleNotif = (notif) => {
            console.log("Received realtime notification:", notif);
            setToastNotif(notif);
    ...
    if (loading || !user || user.role !== 'wali_siswa') {
        return (
            <div className="flex h-screen w-screen items-center justify-center bg-slate-50 dark:bg-[#020c08] text-slate-800 dark:text-white transition-colors duration-300">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-emerald-500 font-medium">Memverifikasi Wali Siswa...</p>
                </div>
            </div>
        );
    }
    ...
    const navigation = [
        { name: 'Dashboard', href: '/wali_siswa/dashboard', icon: LayoutDashboard },
        { name: 'Jadwal Pelajaran', href: '/wali_siswa/jadwal', icon: CalendarDays },
        { name: 'Kehadiran Siswa', href: '/wali_siswa/akademik', icon: BookOpen },
        { name: 'Nilai Akademik', href: '/wali_siswa/nilai', icon: GraduationCap },
        { name: 'Kedisiplinan', href: '/wali_siswa/kedisiplinan', icon: Shield },
        { name: 'Keuangan', href: '/wali_siswa/keuangan', icon: CircleDollarSign },
    ];
        ...
        <div className="h-screen overflow-hidden bg-slate-50 dark:bg-[#020c08] text-slate-800 dark:text-slate-100 flex flex-col md:flex-row transition-colors duration-300">
            ...
            {toastNotif && (
                <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[100] bg-white text-slate-700 px-5 py-4 rounded-2xl shadow-xl flex items-start gap-4 animate-fade-in-up max-w-[90vw] md:max-w-md w-full border border-slate-200">
                    ...
                    <button onClick={() => setToastNotif(null)} className="shrink-0 text-slate-400 hover:text-slate-700 transition-colors">
                        <X className="h-5 w-5" />
                    </button>
                </div>
            )}
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-y-auto overflow-x-hidden">
                ...
                <header className={`flex items-center justify-between px-6 border-b border-slate-200 dark:border-emerald-500/10 bg-white/80 dark:bg-[#020c08]/50 backdrop-blur-md sticky top-0 z-30 transition-all duration-300 ${pathname.endsWith('/dashboard') ? 'h-24 py-2' : 'h-20'}`}>
                    <div className="flex items-center gap-0 md:gap-3 flex-1 min-w-0 md:flex-none">
                        ...
                        {/* Child display */}
                        {loadingChildren ? (
                            <div className="h-5 w-32 bg-slate-200 dark:bg-white/5 animate-pulse rounded-md"></div>
                            ...
                            <span className="text-xs text-red-500 font-bold">Wali belum dipetakan ke siswa</span>
                        )}
                    </div>
                            ...
                            >
                                ...
                                <div className="h-10 w-10 rounded-full bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center font-bold text-emerald-600 dark:text-emerald-400 text-sm overflow-hidden">
                                    {user?.avatar ? (
                                        ...
                                        user?.nama_lengkap?.substring(0, 2).toUpperCase() || 'WL'
                                    )}
                                </div>
                            </button>
                                ...
                                <>
                                    ...
                                    <div className="absolute right-0 mt-2.5 w-60 rounded-2xl bg-white dark:bg-[#041610] border border-slate-200 dark:border-emerald-500/10 shadow-xl py-2 z-50 animate-fade-in text-slate-800 dark:text-slate-200">
                                        <div className="px-4 py-2 border-b border-slate-100 dark:border-emerald-500/5">
                                            <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Wali Murid</p>
...
export default function WaliLayout({ children }) {
    return (
        <ChildProvider>
            <WaliLayoutContent>{children}</WaliLayoutContent>
        </ChildProvider>
    );
}
