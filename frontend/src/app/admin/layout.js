...
import { 
    ...
    Printer, ClipboardList, Archive, Activity, ArrowRightLeft, ChevronDown
} from 'lucide-react';
...
export default function AdminLayout({ children }) {
    ...
    const [openSubmenus, setOpenSubmenus] = useState({
        'Laporan & Lainnya': ['/admin/cetak_raport', '/admin/ekstrakurikuler', '/admin/mutasi'].includes(pathname)
    });
        ...
        return (
            <div className="flex h-screen w-screen items-center justify-center bg-slate-50 dark:bg-[#020c08] text-slate-800 dark:text-white transition-colors duration-300">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-emerald-500 font-medium">Memverifikasi Admin...</p>
                </div>
            </div>
        );
    }
    ...
    const navigationGroups = [
        ...
        },
        {
            ...
            items: [
                ...
                { name: 'Data Kelas', href: '/admin/kelas', icon: Building },
                { name: 'Mata Pelajaran', href: '/admin/mapel', icon: BookOpenCheck },
        ...
        },
        {
            ...
            items: [
                { name: 'Rekap Nilai Kelas', href: '/admin/rekap_nilai', icon: ClipboardList },
                { name: 'Cetak Raport', href: '/admin/cetak_raport', icon: Printer },
            ]
        },
        {
            title: "Sistem",
    ...
    const getPageTitle = () => {
        ...
        if (!isNaN(lastPart) && pathParts.length >= 2) {
            const parentPart = pathParts[pathParts.length - 2];
            if (parentPart === 'ekstrakurikuler') return 'Detail Ekstrakurikuler';
            if (parentPart === 'kelas') return 'Detail Kelas';
            return 'Detail Data';
        }
    ...
    return (
        <div className="h-screen overflow-hidden bg-white dark:bg-[#020c08] text-slate-800 dark:text-slate-100 flex flex-col md:flex-row transition-colors duration-300">
                                    ...
                                    return (
                                        <div key={item.name} className="space-y-1">
                                            <button
                                                ...
                                                className={`
                                                    w-full flex items-center justify-between rounded-xl px-3 py-2.5 text-sm font-semibold transition-all duration-200 cursor-pointer
                                                    ${isParentActive 
                                            ...
                                            {isOpen && !isCollapsed && (
                                                <div className="pl-6 space-y-1 animate-fade-in mt-1">
                                                    {item.children.map((child) => {
                                                        ...
                                                        return (
                                                            <Link
                                                                ...
                                                                className={`
                                                                    flex items-center gap-3 rounded-lg px-4 py-2 text-xs font-semibold transition-all duration-200
                                                                    ${isChildActive 
                                                                ...
                                                                `}
                                                            >
                                                                <span className={`h-1.5 w-1.5 rounded-full shrink-0 ${isChildActive ? 'bg-emerald-500' : 'bg-slate-400'}`} />
                                ...
                                <>
                                    ...
                                    <div className="absolute right-0 mt-2.5 w-60 rounded-2xl bg-white dark:bg-[#041610] border border-slate-200 dark:border-emerald-500/10 shadow-xl py-2 z-50 animate-fade-in text-slate-800 dark:text-slate-200">
                                        <div className="px-4 py-2 border-b border-slate-100 dark:border-emerald-500/5">
                                            <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Admin Panel</p>
